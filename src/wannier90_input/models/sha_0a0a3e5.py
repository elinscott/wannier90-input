...
class Wannier90Input(BaseModel):
    ...
    gamma_only: bool = Field(
        False,
        description="Wavefunctions from underlying ab initio calculation are manifestly real",
    )
    ...
    search_shells: int = Field(
        36,
        description="The number of shells to search when determining finite difference formula",
    )
    ...
    kmesh_tol: float = Field(
        1e-06,
        description="The tolerance to control if two kpoint belong to the same shell",
    )
    higher_order_n: int = Field(
        1,
        description="The order of higher-order finite difference to get b-vectors and weights",
    )
    ...
    write_xyz: bool = Field(
        False,
        description="To write atomic positions and final centres in xyz file format",
    )
    ...
    dis_num_iter: int = Field(
        200,
        description="Number of iterations for the minimisation of $\\Omega_{\\mathrm{I}}$",
    )
    dis_mix_ratio: float = Field(
        0.5,
        description="Mixing ratio during the minimisation of $\\Omega_{\\mathrm{I}}$",
    )
    ...
    slwf_lambda: float = Field(
        0.0,
        description="Value of the Lagrange multiplier for constraining the objective WFs",
    )
    ...
    write_bvec: bool = Field(
        False,
        description="Write to file the matrix elements of the bvectors and their weights",
    )
    write_tb: bool = Field(
        False,
        description="Write lattice vectors, Hamiltonian, and position operator in WF basis",
    )
    ...
    dist_cutoff_mode: Literal["three_dim", "two_dim", "one_dim"] = Field(
        "three_dim",
        description="Dimension in which the distance between WF is calculated",
    )
    ...
    ws_distance_tol: float = Field(
        1e-05,
        description="Absolute tolerance for the distance to equivalent positions.",
    )
    ...
    write_u_matrices: bool = Field(
        False,
        description="Write $U^{(\\bm{k})}$ and $U^{dis(\\bm{k})}$ matrices to files",
    )
