...
class Wannier90Input(BaseModel):
    ...
    mp_grid: tuple[int, int,
                   int] = Field(..., description="Dimensions of the Monkhorst-Pack grid of k-points")
    ...
    nnkpts: Annotated[NDArray[Shape["*, 5"], int],
                      Field(default_factory=list, description="Explicit list of nearest-neighbour k-points")]
    ...
    translate_home_cell: bool = Field(
        False, description="To translate final Wannier centres to home unit cell when writing xyz file")
    ...
    write_hr_diag: bool = Field(
        False, description="To write the diagonal elements of the Hamiltonian in the Wannier basis to `seedname.wout` (in eV)")
    ...
    dis_conv_window: int = Field(
        3, description="The number of iterations over which convergence of $\\Omega_{\\mathrm{I}}$ is assessed.")
    ...
    dis_spheres: list[DisentanglementSphere] = Field(
        default_factory=list, description="List of centres and radii, for disentanglement only in spheres")
    num_iter: int = Field(100, description="Number of iterations for the minimisation of $\\Omega$")
    num_cg_steps: int = Field(
        5, description="During the minimisation of $\\Omega$ the number of Conjugate Gradient steps before resetting to Steepest Descents")
    ...
    conv_noise_amp: float = Field(
        -1.0, description="The amplitude of random noise applied towards end of minimisation procedure")
    ...
    trial_step: float = Field(
        2.0, description="The trial step length for the parabolic line search during the minimisation of $\\Omega$")
    fixed_step: float = Field(
        -999.0, description="The fixed step length to take during the minimisation of $\\Omega$, instead of doing a parabolic line search")
    ...
    slwf_centres: list[CentreConstraint] = Field(
        default_factory=list, description="The centres to which the objective WFs are to be constrained")
    ...
    use_ws_distance: bool = Field(
        True, description="Improve interpolation using minimum distance between WFs, see Chap. [Some notes on the interpolation](notes_interpolations.md)")
    ...
    ws_search_size: int = Field(
        2, description="Maximum extension in each direction of the super-cell of the Born-von Karmann cell to search for points inside the Wigner-Seitz cell")
    ...
    tran_win_min: float = Field(-3.0,
                                description="Bottom of the energy window for transport calculation")
