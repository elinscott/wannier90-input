...
class Wannier90Input(BaseModel):
    ...
    exclude_bands: list[int] = Field(default_factory=list, description="List of bands to exclude from the calculation")
    select_projections: list[int] = Field(default_factory=list, description="List of projections to use in Wannierisation")
    auto_projections: bool = Field(False, description="To automatically generate initial projections")
    restart: Literal["default", "wannierise", "plot", "transport"] = Field("default", description="Restart from checkpoint file")
    iprint: int = Field(1, description="Output verbosity level")
    length_unit: Literal["Ang", "Bohr"] = Field("Ang", description="System of units to output lengths")
    wvfn_formatted: bool = Field(False, description="Read the wavefunctions from a (un)formatted file")
    spin: Literal["up", "down"] = Field("up", description="Which spin channel to read")
    timing_level: int = Field(1, description="Determines amount of timing information written to output")
    optimisation: int = Field(3, description="Optimisation level")
    translate_home_cell: bool = Field(False, description="To translate final Wannier centres to home unit cell when writing xyz file")
    write_xyz: bool = Field(False, description="To write atomic positions and final centres in xyz file format")
    write_vdw_data: bool = Field(False, description="To write data for futher processing by w90vdw utility")
    write_hr_diag: bool = Field(False, description="To write the diagonal elements of the Hamiltonian in the Wannier basis to `seedname.wout` (in eV)")
    ...
    dis_froz_min: float | None = Field(None, description="Bottom of the inner (frozen) energy window")
    ...
    dis_proj_min: float = Field(0.01, description="Lower threshold for projectability disentanglement")
    dis_proj_max: float = Field(0.95, description="Upper threshold for projectability disentanglement")
    dis_num_iter: int = Field(200, description="Number of iterations for the minimisation of $\\Omega_{\\mathrm{I}}$")
    dis_mix_ratio: float = Field(0.5, description="Mixing ratio during the minimisation of $\\Omega_{\\mathrm{I}}$")
    dis_conv_tol: float = Field(1e-10, description="The convergence tolerance for finding $\\Omega_{\\mathrm{I}}$")
    dis_conv_window: int = Field(3, description="The number of iterations over which convergence of $\\Omega_{\\mathrm{I}}$ is assessed.")
    dis_spheres_num: int = Field(0, description="Number of spheres in k-space where disentaglement is performed")
    dis_spheres_first_wann: int = Field(1, description="Index of the first band to be considered a Wannier function")
    dis_spheres: list[DisentanglementSphere] = Field(default_factory=list, description="List of centres and radii, for disentanglement only in spheres")
    num_iter: int = Field(100, description="Number of iterations for the minimisation of $\\Omega$")
    num_cg_steps: int = Field(5, description="During the minimisation of $\\Omega$ the number of Conjugate Gradient steps before resetting to Steepest Descents")
    conv_window: int = Field(3, description="The number of iterations over which convergence of $\\Omega$ is assessed")
    ...
    conv_noise_amp: float = Field(-1.0, description="The amplitude of random noise applied towards end of minimisation procedure")
    ...
    num_no_guide_iter: int = Field(0, description="The number of iterations after which guiding centres are used")
    trial_step: float = Field(2.0, description="The trial step length for the parabolic line search during the minimisation of $\\Omega$")
    fixed_step: float = Field(-999.0, description="The fixed step length to take during the minimisation of $\\Omega$, instead of doing a parabolic line search")
    ...
    wannier_plot_format: Literal["xcrysden", "cube"] = Field("xcrysden", description="File format in which to plot the WF")
    wannier_plot_mode: Literal["crystal", "molecule"] = Field("crystal", description="Mode in which to plot the WF, molecule or crystal")
    ...
    wannier_plot_spinor_mode: Literal["total", "up", "down"] = Field("total", description="Quantity to plot for spinor WF")
    wannier_plot_spinor_phase: bool = Field(True, description="Include the “phase” when plotting spinor WF")
    bands_plot: bool = Field(False, description="Plot interpolated band structure")
    kpoint_path: list[tuple[SpecialPoint, SpecialPoint]] = Field(default_factory=list, description="K-point path for the interpolated band structure")
    bands_num_points: int = Field(100, description="Number of points along the first section of the k-point path")
    bands_plot_format: Literal["gnuplot", "xmgrace"] = Field("gnuplot", description="File format in which to plot the interpolated bands")
    bands_plot_project: list[int] = Field(default_factory=list, description="WF to project the band structure onto")
    bands_plot_mode: Literal["s-k", "cut"] = Field("s-k", description="Slater-Koster type interpolation or Hamiltonian cut-off")
    bands_plot_dim: Annotated[int, Field(ge=1, le=3)] = Field(3, description="Dimension of the system")
    fermi_surface_plot: bool = Field(False, description="Plot the Fermi surface")
    fermi_surface_num_points: int = Field(50, description="Number of points in the Fermi surface plot")
    fermi_energy: float = Field(0.0, description="The Fermi energy")
    fermi_energy_min: float | None = Field(None, description="Lower limit of the Fermi energy range")
    fermi_energy_max: float | None = Field(None, description="Upper limit of the Fermi energy range")
    fermi_energy_step: float = Field(0.01, description="Step for increasing the Fermi energy in the specified range")
    fermi_surface_plot_format: Literal["xcrysden"] = Field("xcrysden", description="File format for the Fermi surface plot")
    hr_plot: bool = Field(False, description="This parameter is not used anymore. Use write_hr instead.")
    ...
    write_bvec: bool = Field(False, description="Write to file the matrix elements of the bvectors and their weights")
    write_tb: bool = Field(False, description="Write lattice vectors, Hamiltonian, and position operator in WF basis")
    ...
    write_u_matrices: bool = Field(False, description="Write $U^{(\\bm{k})}$ and $U^{dis(\\bm{k})}$ matrices to files")
    transport: bool = Field(False, description="Calculate quantum conductance and density of states")
    transport_mode: Literal["bulk", "lcr"] = Field("bulk", description="Bulk or left-lead_conductor_right-lead calculation")
    tran_win_min: float = Field(-3.0, description="Bottom of the energy window for transport calculation")
    tran_win_max: float = Field(3.0, description="Top of the energy window for transport calculation")
    ...
    tran_num_lc: int = Field(0, description="Number of columns in a left-lead_conductor Hamiltonian")
    ...
    tran_num_bandc: int = Field(0, description="Half-bandwidth+1 of a band-diagonal conductor Hamiltonian")
    tran_write_ht: bool = Field(False, description="Write the Hamiltonian for transport calculation")
